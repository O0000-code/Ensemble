# Sidebar Reorder — Technical Plan V2（技术规划 — 评审后修订版）

> **Decisional 文档**。技术细节冲突时以本文档为准。设计层冲突按 `02_design_spec.md`。
> V1 已归档至 `_archive/v1/03_tech_plan.md`。
> V2 修订基于 5 份评审报告，处理 8 个 Architecture P0 + 3 个 Regression P0。
...
## 7. DndContext 配置（V2 — restrictToVerticalAxis 不应用 DragOverlay）
...
<DndContext
  ...
  onDragEnd={(e) => {
    ...
    // V2 设置 justDroppedRef in row to prevent navigate click
    setJustDroppedId(String(e.active.id));
    setTimeout(() => setJustDroppedId(null), 50);  // next tick
  }}
...
>
  <SortableContext
    ...
    disabled={isAdding || editingId !== null}  // V2 — 整体 disable，覆盖 add input 边界
  >
  ...
  {/* DragOverlay 不应用 modifiers，避免 P0-4 跟手错位 */}
...
<DndContext
  sensors={sensors}
  collisionDetection={closestCenter}
  modifiers={[restrictToParentElement, snapModifier]}  // V2: 无 restrictToVerticalAxis
  ...
>
  ...
  <DragOverlay dropAnimation={TAG_DROP_ANIMATION}>
...
## 11. Snap Modifier（V2 新增 — 12px 磁吸）
...
/**
 * Snap the dragged item's center to the nearest droppable's center
 * when within SNAP_DISTANCE_PX pixels. Smooth via CSS transition (no extra animation).
 */
export const snapModifier: Modifier = ({ transform, draggingNodeRect, droppableContainers, over }) => {
  ...
  // over.rect = current drop target rect
