# Sidebar Reorder — Design Spec V2（设计规格 — 评审后修订版）

> **Decisional 文档**。视觉/动效细节冲突时以本文档为准。
> V1 已归档至 `_archive/v1/02_design_spec.md`。
> V2 修订基于 5 份评审报告（`05_review/01-05_*.md`），处理 6 个 design P0、4 个 animation P0。
...
### 2.1 Lift（拾起）—— 拖拽激活的瞬间（V2 改）

> V2 改为两段动画：**吸盘 80ms + 拉离 120ms**（共 200ms），模拟 Things 3 物理感。用户感受："鼠标先把它'吸住'，再'拉离'桌面"。

| 阶段 | 时长 | Categories | Tags |
|---|---|---|---|
| **激活手势** | — | 鼠标按下后移动 ≥ **4px**（HIG "about three points"） | 同 |
| **激活延迟** | — | 0ms（无 long-press） | 同 |
| **吸盘 sub-stage**<br>0–80ms | 80ms | 原位 scale 1.0 → 1.04（轻微胀） | 1.0 → 1.06 |
| **拉离 sub-stage**<br>80–200ms | 120ms | 原位 scale 1.04 → 1.0、原位 opacity 1.0 → **0**（消失让位） | 同 |
| 原位 transform-origin | center | center |
| 原位 timing 函数 | — | `var(--ease-drag-lift)` = `cubic-bezier(0.34, 1.32, 0.64, 1)` | 同 |
| Cursor 切换时机 | — | **激活瞬间**（4px 阈值达到时） → `grabbing` | 同 |

**V2 核心变更解释**：
- **原位 opacity → 0**（D-P0-1 修复）：Finder/Notes 等 macOS 原生在 NSOutlineView 行为是"消失让位"。原位 0.4 是 Atlassian 风格，会形成"原位+overlay+cascade"三重视觉冗余。
- **两段 lift**（A-P0-6 修复）：单段 1.02 scale 在 32px 行上 = 0.64px 视觉增量，肉眼几乎不可感知。改为吸盘式 1.04 → 1.0 + 立即消失，用户感知"被吸起的物理过程"。
...
### 2.4 Cascade（让位）—— 其他项响应（V2 修正 spring 等价）

> V2 修正 A-P0-1：原 V1 的 `spring(stiffness:600, damping:38)` 是 drop-indicator 用的微调参数，与 cubic-bezier(0.16, 1, 0.3, 1) 220ms **不等价**。V2 改为正确的等价值。

| 属性 | Categories（1D） | Tags（2D wrap） |
|---|---|---|
| **CSS timing**（默认实现路径） | `220ms cubic-bezier(0.16, 1, 0.3, 1)` | `220ms cubic-bezier(0.16, 1, 0.3, 1)` |
| **Spring 等价**（如改用 motion） | `{ stiffness: 500, damping: 40, mass: 1 }`（damping ratio ≈ 0.89，无可见 overshoot） | `{ stiffness: 460, damping: 40, mass: 1 }`（damping ratio ≈ 0.93） |
| **react-spring 等价** | `{ tension: 350, friction: 32 }` | `{ tension: 320, friction: 32 }` |
| stagger | **0** | **0** |
| GPU | 仅用 `transform: translate*` | 同 |

> **V2 注意**：Tags cascade 时长由 V1 的 240ms 改为 **220ms**（与 Categories 一致），减少视觉割裂。wrap 重排的"稳定性"通过算法（`closestCenter` + `MeasuringStrategy.Always`）保证，不通过延长动画。

### 2.5 Snap 磁吸（V2 新增）

> **D-P0-2 / A-P0-4 修复**：用户原文明确要"磁吸"，V1 缺失。V2 新增完整规格。

**触发条件**：被拖项几何中心距最近 slot 中心 ≤ **12px**（CSS pixels）

**视觉**：DragOverlay 在 mouseup **之前**就开始平滑吸附到 slot 中心，时长 **80ms `ease-out`**

**反向**：吸附状态下，pointer 移动 > 4px 离开吸附区时立即解除（DragOverlay 恢复纯跟手）

**实现**：dnd-kit 内置 `closestCenter` collision detection 已暴露 `over.rect`，在 `useDraggable` 的 transform 上加 modifier 当距离阈值内时强制对齐 slot 中心。
...
### 2.6 Settle（落定）—— drop 完成（V2 改时长）

> **A-P0-3 修复**：V1 settle 180ms < cascade 220ms 会有目标抖动；V2 改为 settle 220ms（与 cascade 等长）

| 属性 | 值 |
|---|---|
| DragOverlay 滑向最终位置 | dnd-kit `dropAnimation`：`{ duration: 220, easing: 'cubic-bezier(0.16, 1, 0.3, 1)' }` |
| 原位 opacity 恢复 | 0 → 1.0 与 dropAnimation 同步 |
| 总 settle 时长 | **220ms** |
| overshoot | **无** |
| 触觉反馈 | 不实现 |

### 2.7 Cancel（取消）（V2 spring 微 overshoot）

> A-P1 优化：橡皮筋应有 ~1% overshoot 的物理感

| 触发 | 反馈 |
|---|---|
| 按 Esc 键 | DragOverlay snap-back 到原位（`280ms`，等价 spring `{stiffness:280, damping:32}` 即 ζ≈0.96，~0.5% overshoot）；原位 opacity 0 → 1 |
...
2. ☐ Lift 两段总时长 = **200ms ±5ms**（80 grip + 120 pull）
3. ☐ Cascade 让位 duration = **220ms ±5ms**，FPS 全程 **≥ 55**（DevTools FPS 仪表）
4. ☐ Settle 完成时间 = **220ms ±5ms**，无可见 overshoot（最大 transform delta < 0.5%）
5. ☐ 磁吸触发距离 = **12px ±1px**，吸附时长 = **80ms ±5ms**
6. ☐ Drop indicator 颜色 = `var(--color-accent)` 解析后的 `#0063E1`（light）
7. ☐ Drop indicator 在 wrap 末端正确显示短水平线（24×2px）
8. ☐ Cancel snap-back duration = **280ms ±5ms**，spring overshoot ≤ 1%
